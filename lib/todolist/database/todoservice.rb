...
module Todolist
  module Database
    ...
    # TodoService is a wrapper around Todolist::Database::Todo to do use case
    # operations. For all methods, Todolist::Todo instances will be returned
    # instead of Todolist::Database::Todo instances that inherit from
    # Sequel::Model.
    class TodoService
      ...
      def get(id)
        todo = Todolist::Database::Todo[id]
      ...
      def insert(todo)
        todo = Todolist::Database::Todo.new(
          title: todo.title,
      ...
      def update(todo)
        todo = Todolist::Database::Todo[todo.id].set(
          title: todo.title,
          description: todo.description,
          done: todo.status == :finished
        )
      ...
      def delete(todo)
        Todolist::Database::Todo[todo.id].destroy.as_entity
      end
      ...
      def delete_finished
        Todolist::Database::Todo.where(done: true).delete
      end
    end
  end
end
